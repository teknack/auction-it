<?php 
    ...
    $sql = "SELECT `tek_name` , `u_itemswon`, `u_itempoints` FROM `user` WHERE `u_itempoints` > 0 ORDER BY `u_itempoints` LIMIT 10";
    ...
    if (!mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            if (isset($row["tek_name"])) {
                $i++;
                $tek_name[$i] = $row["tek_name"];
                $no_item_won[$i] = $row["u_itemswon"];
                $item_points[$i] = $row["u_itempoints"];
            }else{
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    <ul class="nav navbar-nav">
                        ...
                        <li class="active"><a href="#">Auction Winner<span class="sr-only">(current)</span></a></li>
                    </ul>
                            ...
                            <?php } else { ?>
                             <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th>Teknack Name</th>
                                        <th>No. of item win</th>
                                        <th>Auction points</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    ...
                                    while($i > -1) { ?>
                                        <tr>
                                            <td><?php echo $tek_name[$i]; ?></td>
                                            <td><?php echo $no_item_won[$i]; ?></td>
                                            <td><?php echo $item_points[$i]; ?></td>
                                        </tr>
                                    <?php $i--; } ?>
                                </tbody>
                            </table>
